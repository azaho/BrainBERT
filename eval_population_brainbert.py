...
parser.add_argument('--random_init', action='store_true', help='Whether to use random initialization instead of pretrained weights')
...
def get_brainbert_features(x, fs=2048, max_batch_size=12):
    ...
    with torch.no_grad():
        for i in range(0, batch_size, max_batch_size):
            ...
            log(f"Forwarding batch {i//max_batch_size+1} of {(batch_size-1)//max_batch_size+1}", priority=4, indent=3)
            ...
            if time_granularity > 0:
                ...
                batch_out = batch_out.reshape(batch_i, n_channels, n_timebins, timebin_size, d_model)
                batch_out = batch_out.mean(dim=3)  # (batch_i, n_channels, n_timebins, d_model)
            else:
                n_timebins = n_times
            ...
            if out_array is None:
                out_array = np.empty((batch_size, n_channels, n_timebins, d_model), dtype=batch_np.dtype)
            ...
            del batch_inputs, batch_mask, batch_out, batch_np, linear, batch_x, f, t
