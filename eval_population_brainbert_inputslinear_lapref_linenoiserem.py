...
def get_stft(x, fs, clip_fs=-1, normalizing=None, boundary=None, clip=0, **kwargs):
    ...
    freqs_to_filter = [60, 120, 180, 240, 300, 360]
...
def get_brainbert_features(x, fs=2048, max_batch_size=12):
    ...
    from laplacian_rereferencing import laplacian_rereference_neural_data
    log("Laplacian rereferencing...", priority=3, indent=2)
    x_torch = torch.FloatTensor(x)
    x_torch, _ = laplacian_rereference_neural_data(x_torch, subject.electrode_labels, remove_non_laplacian=True)
    x = x_torch.cpu().numpy()
