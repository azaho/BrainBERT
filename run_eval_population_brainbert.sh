...
echo "Command: python -u eval_population_brainbert.py --eval_name $EVAL_NAME --subject $SUBJECT --trial $TRIAL --splits_type $SPLITS_TYPE --verbose --save_dir $SAVE_DIR --lite --only_1second --time_granularity $TIME_GRANULARITY"

# Add the -u flag to Python to force unbuffered output
/om2/user/zaho/BrainBERT/.venv/bin/python -u eval_population_brainbert.py --eval_name $EVAL_NAME --subject $SUBJECT --trial $TRIAL --splits_type $SPLITS_TYPE --verbose --save_dir $SAVE_DIR --lite --only_1second --time_granularity $TIME_GRANULARITY