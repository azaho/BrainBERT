...
SAVE_DIR="eval_results_${SPLITS_TYPE}_multiclass"
...
echo "Command: python -u run_neuroprobe_eval_frozen_population.py --only_1second --eval_name $EVAL_NAME --subject $SUBJECT --trial $TRIAL --split_type $SPLITS_TYPE --save_dir $SAVE_DIR --feature_type $FEATURE_TYPE --randomly_initialized_model $RANDOM_INIT --binary_tasks False"

# Add the -u flag to Python to force unbuffered output
python -u run_neuroprobe_eval_frozen_population.py --only_1second --eval_name $EVAL_NAME --subject_id $SUBJECT --trial_id $TRIAL --split_type $SPLITS_TYPE --save_dir $SAVE_DIR --feature_type $FEATURE_TYPE --randomly_initialized_model $RANDOM_INIT --binary_tasks False